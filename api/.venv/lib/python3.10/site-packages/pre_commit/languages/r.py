...
def _execute_r(
    ...
    *,
    ...
    with in_env(prefix, version), _r_code_in_tempfile(code) as f:
        _, out, _ = cmd_output(_rscript_exec(), *cli_opts, f, *args, cwd=cwd)
    return out.rstrip('\n')
...
def _prefix_if_file_entry(
    entry: list[str], prefix: Prefix, *, is_local: bool
) -> Sequence[str]:
...
def _cmd_from_hook(
    prefix: Prefix, entry: str, args: Sequence[str], *, is_local: bool
) -> tuple[str, ...]:
...
def run_hook(
    ...
    *,
    ...
    return lang_base.run_xargs(
        cmd, file_args, require_serial=require_serial, color=color
    )
