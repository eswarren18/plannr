...
def _check_hooks_still_exist_at_rev(
    repo_config: dict[str, Any], info: RevInfo
) -> None:
...
def _update_one(
    i: int, repo: dict[str, Any], *, tags_only: bool, freeze: bool
...
def _original_lines(
    path: str, rev_infos: list[RevInfo | None], retry: bool = False
...
def autoupdate(
    ...
    with concurrent.futures.ThreadPoolExecutor(jobs) as exe:
        futures = [
            exe.submit(
                _update_one, i, repo, tags_only=tags_only, freeze=freeze
            )
