...
class Binding:
    ...
    def __repr__(self):
        return '<{} object {!r} from line {!r} at 0x{:x}>'.format(
            self.__class__.__name__, self.name, self.source.lineno, id(self)
        )
...
class Checker:
    ...
    def addBinding(self, node, value):
        ...
        if value.name not in self.scope or not isinstance(value, Annotation):
            if isinstance(value, NamedExprAssignment):
                ...
                if value.name in scope and isinstance(
                    scope[value.name], Annotation
                ):
    ...
    def _handle_percent_format(self, node):
        try:
            ...
            self.report(
                messages.PercentFormatInvalidFormat, node, 'incomplete format'
            )
            return
        ...
        for _, placeholder in placeholders:
            ...
            for part in (width, precision):
                if part is not None and '*' in part:
                    if not positional:
                        self.report(
                            messages.PercentFormatStarRequiresSequence, node
                        )
                    else:
                        positional_count += 1
            ...
            if positional and name is not None:
                self.report(
                    messages.PercentFormatMixedPositionalAndNamed, node
                )
                return
    ...
    def LAMBDA(self, node):
        ...
        def runFunction():
            with self.in_scope(FunctionScope):
                self.handleChildren(
                    node, omit=('decorator_list', 'returns', 'type_params')
                )
