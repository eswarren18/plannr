...
def normalize_fstring_quotes(
    quote: str, middles: list[Leaf], is_raw_fstring: bool
) -> tuple[list[Leaf], str]:
    """Prefer double quotes but only if it doesn't cause more escaping.
    ...
    for middle in middles:
        ...
        new_segment = sub_twice(
            escaped_new_quote, rf"\1\2{new_quote}", segment
        )
        ...
        new_segment = sub_twice(
            escaped_orig_quote, rf"\1\2{quote}", new_segment
        )
        new_segment = sub_twice(
            unescaped_new_quote, rf"\1\\{new_quote}", new_segment
        )
