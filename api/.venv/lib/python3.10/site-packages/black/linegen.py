            ...
            if child.type == token.RARROW:
                ...
                if (
                    ...
                    if maybe_make_parens_invisible_in_atom(
                        child, parent=node, remove_brackets_around_comma=False
                    ):
                        wrap_in_parentheses(node, child, visible=False)
                else:
                    wrap_in_parentheses(node, child, visible=False)
...
def _first_right_hand_split(
    line: Line, omit: Collection[LeafID] = ()
) -> RHSResult:
        ...
        try:
            ...
            if line.is_chained_assignment:
                ...
                and not (
                    ...
                    and rhs.opening_bracket.parent.parent.type
                    == syms.dictsetmaker
                )
            ):
                raise CannotSplit(
...
def _ensure_trailing_comma(
    ...
    if any(
        leaf.type == token.COMMA and not is_part_of_annotation(leaf)
        for leaf in leaves
    ):
        return False
...
def delimiter_split(
    ...
    for leaf_idx, leaf in enumerate(line.leaves):
        ...
        if (
            last_leaf.type == STANDALONE_COMMENT
    ...
    for index, child in enumerate(list(node.children)):
        ...
        if check_lpar:
            if (
                ...
                if maybe_make_parens_invisible_in_atom(
                    child, parent=node, remove_brackets_around_comma=True
                ):
                    wrap_in_parentheses(node, child, visible=False)
...
def remove_with_parens(node: Node, parent: Node) -> None:
    ...
    if node.type == syms.atom:
        if maybe_make_parens_invisible_in_atom(
            node, parent=parent, remove_brackets_around_comma=True
        ):
        ...
        if maybe_make_parens_invisible_in_atom(
            node.children[0], parent=node, remove_brackets_around_comma=True
        ):
            wrap_in_parentheses(node, node.children[0], visible=False)


def maybe_make_parens_invisible_in_atom(
    node: LN, parent: LN, remove_brackets_around_comma: bool = False
) -> bool:
...
def generate_trailers_to_omit(
    ...
    for index, leaf, leaf_length in line.enumerate_with_length(
        is_reversed=True
    ):
        length += leaf_length
