...
class Manager:
    ...
    def run_serial(self) -> None:
        ...
        self.results = [
            FileChecker(
                filename=filename, plugins=self.plugins, options=self.options
            ).run_checks()
...
class FileChecker:
    ...
    def __init__(
        self, *, filename: str, plugins: Checkers, options: argparse.Namespace
    ) -> None:
    ...
    def report(
        self, error_code: str | None, line_number: int, column: int, text: str
    ) -> str:
...
def _try_initialize_processpool(
    job_count: int, argv: Sequence[str]
