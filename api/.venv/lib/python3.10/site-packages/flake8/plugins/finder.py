...
def _parse_option(
    cfg: configparser.RawConfigParser, cfg_opt_name: str, opt: str | None
) -> list[str]:
...
def _flake8_plugins(
    eps: Iterable[importlib.metadata.EntryPoint], name: str, version: str
) -> Generator[Plugin]:
...
def _check_required_plugins(
    plugins: list[Plugin], expected: frozenset[str]
) -> None:
    plugin_names = {
...
def find_plugins(
    cfg: configparser.RawConfigParser, opts: PluginOptions
) -> list[Plugin]:
...
def _import_plugins(
    plugins: list[Plugin], opts: PluginOptions
) -> list[LoadedPlugin]:
...
def _classify_plugins(
    plugins: list[LoadedPlugin], opts: PluginOptions
) -> Plugins:
    tree = []
    logical_line = []
    physical_line = []
    ...
    return Plugins(
        checkers=Checkers(
            tree=tree, logical_line=logical_line, physical_line=physical_line
        ),
        reporters=reporters,
        disabled=disabled,
    )
