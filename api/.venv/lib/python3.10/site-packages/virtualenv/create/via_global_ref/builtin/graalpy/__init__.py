...
class GraalPy(ViaGlobalRefVirtualenvBuiltin, ABC):
    ...
    def can_describe(cls, interpreter):
        return (
            interpreter.implementation == "GraalVM"
    ...
    def _executables(cls, interpreter):
        ...
        targets = sorted(
            f"{name}{cls.suffix}" for name in cls.exe_names(interpreter)
        )
    ...
    def sources(cls, interpreter):
        ...
        if native_lib.exists():
            yield PathRefToDest(
                native_lib,
                dest=lambda self, s: self.bin_dir.parent / "lib" / s.name,
            )
        ...
        for jvm_dir_name in ("jvm", "jvmlibs", "modules"):
            ...
            if jvm_dir.exists():
                yield PathRefToDest(
                    jvm_dir, dest=lambda self, s: self.bin_dir.parent / s.name
                )
...
class GraalPyWindows(GraalPy, WindowsSupports):
    ...
    def set_pyenv_cfg(self):
        ...
        super().set_pyenv_cfg()
        self.pyenv_cfg["venvlauncher_command"] = (
            self.interpreter.system_executable
        )


__all__ = ["GraalPyPosix", "GraalPyWindows"]
