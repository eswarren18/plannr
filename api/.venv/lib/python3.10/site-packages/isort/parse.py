...
def file_contents(
    ...
    while index < line_count:
        ...
        if (
            line in config.section_comments
        ...
        for statement in statements:
            ...
            line_parts = [
                part
                for part in strip_syntax(import_string).strip().split(" ")
                if part
            ]
            ...
            if "as" in just_imports and (just_imports.index("as") + 1) < len(
                ...
                while "as" in just_imports:
                    ...
                    if type_of_import == "from":
                        ...
                        if (
                            nested_module == as_name
                            and config.remove_redundant_aliases
                        ):
                            pass
                        elif (
                            as_name not in as_map["from"][module]
                        ):  # pragma: no branch
                        ...
                        if associated_comment:
                            ...
                            if (
                                associated_comment in comments
                            ):  # pragma: no branch
                                comments.pop(
            ...
            if type_of_import == "from":
                ...
                for import_name in just_imports:
                    ...
                    if associated_comment:
                        categorized_comments["nested"].setdefault(
                            import_from, {}
                        )[import_name] = associated_comment
                ...
                for module in just_imports:
                    ...
                    if len(out_lines) > max(import_index, +1, 1) - 1:
                        ...
                        while (
                            ...
                            and not config.treat_all_comments_as_code
                            and last.strip()
                            not in config.treat_comments_as_code
                        ):
                            categorized_comments["above"][
                                "straight"
                            ].setdefault(module, []).insert(
                                0, out_lines.pop(-1)
                            )
                            if out_lines:
                                last = out_lines[-1].rstrip()
