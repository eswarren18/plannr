    ...
    for index, line in enumerate(chain(input_stream, (None,))):
        if line is None:
            ...
            if code_sorting and code_sorting_section:
                if is_reexport:
                    output_stream.seek(
                        output_stream.tell() - reexport_rollback
                    )
                    reexport_rollback = 0
            ...
            if (
                ...
                while char_index < len(line):
                    if line[char_index] == "\\":
                        ...
                        if (
                            line[char_index : char_index + len(in_quote)]
                            == in_quote
                        ):
                            in_quote = ""
                            if (
            ...
            if not (in_quote or was_in_quote or in_top_comment):
                if isort_off:
                    ...
                    if (
                        ...
                        if (
                            import_statement.lstrip().startswith(
                                CIMPORT_IDENTIFIERS
                            )
        ...
        if not_imports:
            ...
            if (
                ...
                and not (
                    line.rstrip().endswith(DOCSTRING_INDICATORS)
                    and "=" not in line
                )
            ):
                add_line_separator = line_separator or "\n"
            ...
            if import_section:
                ...
                if not contains_imports:
                    ...
                    trailing_whitespace = import_section[
                        len(import_section.rstrip()) :
                    ]
                    ...
                    if not (
                        import_section.strip() and not sorted_import_section
                    ):
                        if indent:
                            sorted_import_section = (
                                leading_whitespace
                                + textwrap.indent(
                                    sorted_import_section, indent
                                ).strip()
                                + trailing_whitespace
                            )
...
def _indented_config(config: Config, indent: str) -> Config:
    ...
    return Config(
        ...
        import_footers=(
            config.import_footers if config.indented_import_headings else {}
        ),
    )
