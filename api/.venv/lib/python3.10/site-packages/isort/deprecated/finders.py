...
class PathFinder(BaseFinder):
    def __init__(self, config: Config, path: str = ".") -> None:
        ...
        # conda
        self.conda_env = (
            self.config.conda_env or os.environ.get("CONDA_PREFIX") or ""
        )
        if self.conda_env:
        ...
        for prefix in self.paths:
            ...
            if is_module or is_package:
                ...
                for src_path in self.config.src_paths:
                    if (
                        src_path in path_obj.parents
                        and not self.config.is_skipped(path_obj)
                    ):
...
class ReqsBaseFinder(BaseFinder):
    ...
    def _load_names(self) -> List[str]:
        ...
        for path in self._get_files():
            names.extend(
                self._normalize_name(name) for name in self._get_names(path)
            )
        return names
...
class RequirementsFinder(ReqsBaseFinder):
    ...
    def _get_names_cached(cls, path: str) -> List[str]:
        ...
        with chdir(os.path.dirname(path)):
            ...
            result.extend(
                req.name for req in requirements.values() if req.name
            )
