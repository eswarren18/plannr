...
class _Config:
    ...
    def __post_init__(self) -> None:
        ...
        if self.multi_line_output == WrapModes.VERTICAL_GRID_GROUPED_NO_COMMA:  # type: ignore
            ...
            object.__setattr__(
                self, "multi_line_output", vertical_grid_grouped
            )
...
class Config(_Config):
        ...
        for key, value in tuple(combined_config.items()):
            ...
            if key.startswith(KNOWN_PREFIX) and key not in (
                ...
                if maps_to_section in KNOWN_SECTION_MAPPING:
                    ...
                    if (
                        maps_to_section
                        not in combined_config.get("sections", ())
                        and not quiet
                    ):
                        warn(
            ...
            if key.startswith(IMPORT_HEADING_PREFIX):
                import_headings[key[len(IMPORT_HEADING_PREFIX) :].lower()] = (
                    str(value)
                )
    ...
    def is_skipped(self, file_path: Path) -> bool:
        ...
        if (
            self.directory
            and Path(self.directory) in file_path.resolve().parents
        ):
        ...
        if self.skip_gitignore:
            ...
            if (
                ...
                and str(file_path.resolve())
                not in self.git_ls_files[git_folder]
            ):
                return True
...
def _abspaths(cwd: str, values: Iterable[str]) -> Set[str]:
    paths = {
        (
            ...
            and value.endswith(os.path.sep)
            else value
        )
        for value in values
    }
