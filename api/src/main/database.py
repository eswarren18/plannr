...
def get_db():
    if SessionLocal is None:
        raise RuntimeError(
            "SessionLocal is not initialized. Call init_engine_and_session(database_url) first."
        )
    db = SessionLocal()
    try:
        yield db
...
def create_tables():
    if engine is None:
        raise RuntimeError(
            "Engine is not initialized. Call init_engine_and_session(database_url) first."
        )
